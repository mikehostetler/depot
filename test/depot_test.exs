defmodule DepotTest do
  ...
  describe "filesystem without own processes" do
    ...
    test "user can list files", %{filesystem: filesystem} do
      ...
      assert_in_list list, %Depot.Resource{address: %{path: "/test.txt"}, type: :file}
      assert_in_list list, %Depot.Resource{address: %{path: "/test-1.txt"}, type: :file}
    end
  end
  ...
  #     {:ok, filesystem_a} = Depot.Adapter.Local.start(name: :a, root_path: prefix_a)
  #     {:ok, filesystem_b} = Depot.Adapter.Local.start(name: :b, root_path: prefix_b)
